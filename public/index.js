...
function createParticlePool() {
  ...
  while (--i > -1) {
    ...
    p.setAttributeNS(
      null,
      "fill",
      particleColorArray[randomBetween(0, particleColorArray.length - 1)]
    );
    //p.setAttributeNS(null, 'stroke', '#FFF');
    p.setAttributeNS(
      xlinkns,
      "xlink:href",
      particleType[randomBetween(0, particleType.length - 1)].element
    );
    TweenMax.set(p, {
      scale: 0,
      transformOrigin: "50% 50%",
...
function resetHeartLineClones() {
  for (
    var i = 0;
    i < heartLineClonesTl.getChildren(true, false, true).length;
    i++
  ) {
